...
def verify_application(manifest, by_service):
    """Read-only provider contract; mutating acceptance belongs to qualification."""
    harness = next(s for s, role in manifest['roles'].items() if role == 'harness')
    run(['docker', 'exec', by_service[harness]['Id'], 'node',
         '/opt/dsh-build/verify-router-contract.mjs', '--mode', manifest['mode']])
