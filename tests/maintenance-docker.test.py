...
from maintenance.engine import install_labels
...
''')
    ...
    manifest = {'schema': 1, 'root': str(root), 'repository': REPOSITORY, 'branch': 'main',
        'revision': 'a'*40, 'project': project, 'mode': 'remote',
        ...
        'input_paths': [], 'artifact_paths': [], 'source_artifacts': []}
    ...
    subprocess.run(['docker', 'rm', '-f', portal_name], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
